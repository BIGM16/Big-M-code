...
openResourceButtons.forEach((button) => {
  button.addEventListener("click", () => {
    resourceDivs.forEach((div) => {
      div.classList.add("hidden");
    });
    const parentDiv = button.closest(".parent");
    const resourceDiv = parentDiv.querySelector(".ressource");
...
contactForm.addEventListener("submit", function (e) {
  ...
  const formatChoisi = document.querySelector('input[name="format"]:checked').value.trim();
